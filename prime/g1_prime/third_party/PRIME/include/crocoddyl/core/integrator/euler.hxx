...
namespace crocoddyl
{
      ...
      const Eigen::Ref<const VectorXs> &x, const Eigen::Ref<const VectorXs> &u)
  {
    ...
    //     x.tail(nv);
    VectorXs v = x.tail(nv).eval(); 

    if (auto mb = boost::dynamic_pointer_cast<crocoddyl::StateMultibodyBaseTpl<Scalar>>(state_))
    {
      const std::size_t nv_pin = mb->get_nv_pin();
      v.tail(nv - nv_pin).setZero();
    }

    control_->calc(d->control, Scalar(0.), u);
    differential_->calc(d->differential, x, d->control->w);
    // differential_->calcDiff(d->differential, x, d->control->w); // Should be removed in run time !
      ...
      const Eigen::Ref<const VectorXs> &x, const Eigen::Ref<const VectorXs> &u)
  {
    ...
    // Cast to address the parameter dynamics
    std::size_t nv_pin = state_->get_nv();
    if (auto multibody_state = boost::dynamic_pointer_cast<crocoddyl::StateMultibodyBaseTpl<Scalar>>(state_))
    {
      nv_pin = multibody_state->get_nv_pin();
      // std::cout << "params" << nv_pin << std::endl;
    }
    ...
    d->Fx.topRightCorner(nv, nv).block(0, 0, nv_pin, nv_pin).diagonal().array() += Scalar(time_step_); // Parameter dynamics which is of first order
