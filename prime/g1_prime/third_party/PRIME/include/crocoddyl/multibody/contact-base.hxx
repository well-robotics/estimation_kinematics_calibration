...
namespace crocoddyl
{
  ...
  ContactModelAbstractTpl<Scalar>::ContactModelAbstractTpl(
      boost::shared_ptr<StateMultibody> state,
      const pinocchio::ReferenceFrame type, const std::size_t nc)
      : state_(state), nc_(nc), nu_(state->get_nv_pin()), id_(0), type_(type) {}
  ...
  template <typename Scalar>
  void ContactModelAbstractTpl<Scalar>::updateMode(
  ...
  template <typename Scalar>
  const boost::shared_ptr<typename ContactModelAbstractTpl<Scalar>::StateMultibody> &
