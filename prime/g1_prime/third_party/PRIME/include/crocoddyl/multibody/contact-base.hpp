...
namespace crocoddyl
{
  ...
  public:
    ...
    /**
     * @brief Update the contact mode, contact normal and tangent from the LCP forward dynamics
     *
     * @param[in] data   3d contact data
     * @param[in] mode   contact mode from LCP
     * @param[in] normal   contact normal from LCP
     * @param[in] tangent   contact tangent from LCP
     */
    virtual void updateMode(const boost::shared_ptr<ContactDataAbstract> &data,
                            const Scalar &mode,
                            const Vector3s &normal,
                            const Vector3s &tangent);
  ...
  struct ContactDataAbstractTpl : public ForceDataAbstractTpl<_Scalar>
  {
    ...
    ContactDataAbstractTpl(Model<Scalar> *const model,
        ...
        : Base(model, data),
          ...
          dtau_dq(model->get_state()->get_nv_pin(), model->get_state()->get_nv_pin())
    {
      a0.setZero();
      da0_dx.setZero();
      dtau_dq.setZero();
